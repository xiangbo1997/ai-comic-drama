...
export interface StylePack {
  ...
  colorSystem: string;
  /**
   * 英文精炼色彩系统（≤120 chars）：进图像 prompt 用。
   * 只保留色温/饱和度/主色调核心约束，替代把整段中文 colorSystem 塞进英文 prompt 的做法。
   * legacy 平面风格为空串。
   */
  colorSystemEn: string;
  ...
  sceneRules: string;
  /**
   * 英文精炼场景规则（≤120 chars）：进图像 prompt 用。
   * 只保留空间纵深/光影语言核心，替代把整段中文 sceneRules 塞进英文 prompt。
   * legacy 平面风格为空串。
   */
  sceneRulesEn: string;
  /** 英文风格特化负向词（一行）。 */
...
export const STYLE_PACKS: readonly StylePack[] = [
  // ========== 存量常用风格（保留原 anchor 语义 + 轻度增强） ==========
  {
    ...
    colorSystemEn:
      "warm-neutral palette 5000-5400K, medium saturation, flat cel colors, no fluorescent tones",
    moodPalettes:
    ...
    sceneRulesEn:
      "layered foreground-midground-background depth, clean lines with block shadows, soft cinematic single-source light",
    negative:
  ...
  },
  {
    ...
    colorSystemEn:
      "physically accurate color, medium-low saturation 40-60%, cinematic color grading, natural skin tones",
    moodPalettes:
    ...
    sceneRulesEn:
      "realistic perspective and material texture, physically correct lighting and shadows, atmospheric depth",
    negative:
  ...
  },
  {
    ...
    colorSystemEn:
      "high-contrast bold color blocks, saturation 60-80%, ink-black outlines, complementary accent clash",
    moodPalettes:
    ...
    sceneRulesEn:
      "strong perspective and dynamic composition, block shadows with halftone texture, simplified focused backgrounds",
    negative:
  ...
  },
  {
    ...
    colorSystemEn:
      "translucent washes, medium-low saturation 40-60%, soft gentle color temperature, breathing white space",
    moodPalettes:
    ...
    sceneRulesEn:
      "soft bleeding edges define space, breathing negative space, translucent wet light, no hard edges",
    negative:
  ...
  // ========== Toonflow 改编的 5 个完整画风包 ==========
  {
    ...
    colorSystemEn:
      "warm nostalgic palette 4800-5200K, medium-low saturation 50-70%, hand-painted cel colors, no fluorescent",
    moodPalettes:
    ...
    sceneRulesEn:
      "layered depth with grey atmospheric distance, clean lines and block shadows, worn natural materials, soft warm light",
    negative:
  ...
  },
  {
    ...
    colorSystemEn:
      "traditional Chinese color baseline 5000-5600K, saturation 55-70%, ink outlines, same-hue shadows, no fluorescent",
    moodPalettes:
    ...
    sceneRulesEn:
      "oriental atmosphere with negative-space composition, detailed traditional architecture, poetic layered light",
    negative:
  ...
  },
  {
    ...
    colorSystemEn:
      "bright warm palette 4800-5200K, saturation 65-80%, toon-shaded materials, no fluorescent, no heavy shadows",
    moodPalettes:
    ...
    sceneRulesEn:
      "modern urban spaces, rich spatial layers with detailed materials, cinematic soft warm lighting, no heavy shadows",
    negative:
  ...
  },
  {
    ...
    colorSystemEn:
      "cyan-crimson-indigo-gold base, low-saturation Chinese neon, PBR materials, no fluorescent bleed, no digital noise",
    moodPalettes:
    ...
    sceneRulesEn:
      "Chinese eaved architecture with holographic neon, ink-wash cyber layers, symmetric composition, volumetric light through mist",
    negative:
  ...
  },
  {
    ...
    colorSystemEn:
      "cool desaturated high-grade palette 5800-7000K, saturation 30-50%, strong contrast, no neon",
    moodPalettes:
    ...
    sceneRulesEn:
      "modern urban spaces, shallow depth of field with lens optics, dramatic low-key lighting, cool tone with warm accents",
    negative:
