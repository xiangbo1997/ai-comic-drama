 ...
 *
 * 音效清单单一真源：
 * - sfx 可用标签清单由 lib/sfx-library.buildSfxTagListForPrompt() 程序化生成，不再手写副本
 *   （手写版曾与音效库扩容脱节，新增分类/素材永远不会被 LLM 标注）。
 *
...
import { buildSfxTagListForPrompt } from "@/lib/sfx-library";
...
11. sfx: 音效标注（可选数组）。仅当画面有【明确的声音事件】时输出 [{"tag":"glass-shatter","offsetSec":1}]：
   - tag 只能取以下清单里的值。可给【分类 id】（系统自动取该类代表音效）或【具体音效 id】（更精确，优先）：
${buildSfxTagListForPrompt()}
   - offsetSec 为镜内触发秒（0 = 镜头开始）
   - mode: 触发模式（可选）。【环境氛围类（ambient-* 开头，如雨声/街道/虫鸣/风声/篝火）必须写 "ambient"】，表示整场戏持续铺底的环境底噪，系统会按 locationKey 自动铺满该地点的所有分镜并在场景切换处交叉淡化；此时 offsetSec 无意义可省略。打击/碎裂/心跳等【一次性声音事件】一律【省略】该字段（默认单次触发）。
   - 【环境底噪的标注纪律】ambient 是【每个地点标一次】而非每镜标：同一 locationKey 的连续分镜里，只在【该地点的第一镜】标一条 ambient 即可，后续同地点分镜不要重复标（系统自动铺满整场）。
   - 【克制纪律】每镜 ≤2 个；全片约每 10-30 秒 1-2 个；安静/抒情镜一律【省略】该字段（绝大多数分镜没有它）。一次性音效是标点符号不是背景噪音——只标打击/碎裂/心跳等真实声音事件，宁缺毋滥。
...
【断点优先】切分集边界时，优先把边界落在情绪强度含「转折 / 悬疑 / 高潮」的事件卡之后，禁止在「平铺」卡上断集——断口设计是拆集工序的第一优先级，先定断口再切内容。`;
}
