/**
 * 「内心戏外化 / 旁白纪律」方法论 Prompt 层（单一真源）
 *
 * ⚠️ 定位：这是**通用画面纪律，不是改编专属**。
 * 规则最初蒸馏自影视改编方法论（prose→screenplay 外化转换），但其核心约束
 * （description 里禁止出现不可拍摄的心理状态词）与「文本从哪来」无关——
 * 只要产出的是要喂给图像模型的画面描述，就必须遵守。从世界观直接创作时
 * LLM 反而更容易写出「她心中五味杂陈」（没有原文约束，更爱抒情），
 * 图像模型收到这种描述只能画个呆脸。
 *
 * 由三条产出画面描述的管线共享：
 *   (A) 服务层直连解析：script-parse.ts
 *   (B) Agent 解析路径：agent-prompts/script-parser.ts
 *   (C) 世界观创作路径：agent-prompts/drama-script.ts
 *
 * 设计原则：
...
/** 画面纪律 — 内心戏外化规则（把不可拍的心理活动改写成镜头语言，三条管线共享） */
export const EXTERNALIZATION_RULES = `【内心戏外化（画面描述的最高优先级纪律）】
- 内心独白 / 心理活动，一律优先转为「可见动作 / 表情 / 反应镜头」，而不是塞进旁白；旁白是最后手段。
- 心理状态 → 视觉手法映射（照此写 description，别直译心理词）：
