...
export async function POST(
  ...
  try {
    ...
    try {
      // 如果有模型 ID，测试模型可用性；否则只测试连接
      if (effectiveModelId) {
        testResult = await testModelAvailability(
          config.provider.category as ProviderCategory,
...
async function testProviderConnection(
  ...
  switch (slug) {
    // LLM 提供商
    case "deepseek":
      return testDeepSeek(apiKey, baseUrl);
    case "openai":
      return testOpenAI(apiKey, baseUrl);
    case "gemini":
      return testGemini(apiKey);
...
async function testKling(accessKey: string, secretKey: string): Promise<{ success: boolean; message: string }> {
  ...
  // 可灵需要签名，这里简单验证格式
  return { success: true, message: "配置已保存（需实际调用验证）" };
}
