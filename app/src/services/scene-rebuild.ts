/**
 * 分镜批量重建的持久化逻辑（删旧 + 建新 + generationParams 桥接与聚合）。
 *
 * 原本整段写在 `api/projects/[id]/scenes/route.ts` 的 POST 里。MCP 的
 * `script_to_storyboard` 要把脚本落成分镜，走的必须是同一段逻辑——这里面沉淀了
 * 转场/音效/金句花字聚合、beatType 默认冲击效果、以及「重解析后把用户精调的
 * 贴图/滤镜/字幕位置从旧 sceneId 桥接到新 sceneId」这些历经修复的规则。若 MCP
 * 另写一份，用户从对话侧生成的分镜就会静默丢失上述全部效果（本仓库 P0 级雷区）。
 *
 * 故原样抽到 services 层：Web route 与 MCP 共用同一实现，逻辑逐字未改。
 */
...
const log = createLogger("services:scene-rebuild");
 ...
 * 用一批分镜草稿重建项目的分镜列表。
 *
 * 调用方须自行完成鉴权与项目归属校验；本函数只负责持久化。
 * 返回按 order 升序的新建分镜行。
 */
export async function rebuildProjectScenes(
  id: string,
  project: { generationParams: unknown },
  scenes: unknown[]
) {
  ...
  const createdScenes = await prisma.$transaction(async (tx) => {
    ...
    const orderToNewSceneId = new Map(scenesAfter.map((s) => [s.order, s.id]));
