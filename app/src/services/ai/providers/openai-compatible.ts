...
import type { LLMProvider, ImageProvider, AIServiceConfig, LLMMessage, ImageGenerationOptions } from "../types";
...
const SUPPORTED_IMAGE_MODELS = [
  ...
  "flux", "flux-schnell", "flux-dev", "flux-pro", "flux-1",
  "stable-diffusion", "sd-", "sdxl",
  "grok-2-image", "grok-3-imagegen", "grok-image",
  "midjourney", "imagen", "kandinsky", "playground", "ideogram",
  "recraft", "kolors", "cogview", "wanx", "jimeng", "doubao",
];

// 常见的 LLM 文本模型（明确不支持图像生成）
const LLM_ONLY_MODELS = [
  "gpt-3.5", "gpt-4o", "gpt-4-turbo", "gpt-4.1",
  "claude", "gemini", "deepseek", "qwen", "llama", "mistral",
  "grok-2", "grok-3", "grok-4",
  "meta-llama", "moonshot", "glm", "mixtral", "phi",
];
...
export const openaiCompatibleImage: ImageProvider = {
  async generateImage(options, config) {
    ...
    if (!response.ok) {
      ...
      if (response.status === 404 || errorText.includes("invalid_value") || errorText.includes("not found")) {
        throw new Error(
