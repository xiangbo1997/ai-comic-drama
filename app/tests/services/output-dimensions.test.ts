...
describe("resolveOutputDimensions", () => {
  ...
  it("码率始终取质量档位基准（不随画幅变化）", () => {
    expect(resolveOutputDimensions("720p", "16:9").bitrate).toBe("5M");
    expect(resolveOutputDimensions("720p", "1:1").bitrate).toBe("5M");
    expect(resolveOutputDimensions("720p", "9:16").bitrate).toBe("5M");
  });
